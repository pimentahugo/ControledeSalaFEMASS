...
internal class TurmaRepository : ITurmaRepository
{
	...
	public async Task<IList<AlocacaoSala>> GetAlocacoesByDay(DayOfWeek dayOfWeek)
	{
		return await _context.AlocacoesSala
			.Include(p => p.Turma)
				.ThenInclude(p => p.Disciplina)
			.Include(p => p.Sala)
			.Where(p => p.DiaSemana == dayOfWeek).ToListAsync();
	}
}