    ...
    public async Task Handle(AlocarSalasAutomaticamenteCommand request, CancellationToken cancellationToken)
    {
        ...
        foreach (var turma in turmasOrdenadas)
        {
            if (turma.CodigoHorario.HasValue && turma.CodigoHorario.Value != 0)
            {
                ...
                {
                    var salasDisponiveis = await _salaRepository.GetSalasDisponiveisParaAlocacao(new GetSalasDisponiveisDto(turma, horario.Dia, horario.Tempo));

                    var salaAlocada = salasDisponiveis
                        .FirstOrDefault(s => alocacoes.Any(a => a.Turma.Professor == turma.Professor && a.SalaId == s.Id)) ??
                        salasDisponiveis.FirstOrDefault(s => alocacoes.Any(a => a.Turma.Professor == turma.Professor && a.Sala.Bloco == s.Bloco)) ??
